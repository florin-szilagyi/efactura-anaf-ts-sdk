...
| P2.3 | ubl build cmd | 🟢 | `2026-04-11-anaf-cli-P2.3.md` | merged in `3c21a29`; flags + `--from-json` + `--from-yaml`, override flag plumbing; context resolved up-front via `contextService.resolve` to avoid regex violation; `ubl inspect` uses regex-only preview |
